!< VTK_Fortran test.
program write_volatile_vtr
!< VTK_Fortran test: test the writing of *volatile* XML file for enabling some parallel environments.
use penf
...
! to be removed
print "(A)", a_vtk_file%xml_writer%xml_volatile%chars()

test_passed = .true. ! nothing to test yet

print "(A,L1)", new_line('a')//'Are all tests passed? ', all(test_passed)
stop
endprogram write_volatile_vtr
